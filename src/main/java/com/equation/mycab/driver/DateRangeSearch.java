...
public class DateRangeSearch {
	...
	public DateRangeSearch(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		super();
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
	...
	public void searchData(String from, String to, String currency1) {
		String query = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM cashin,driver WHERE cashin.datePosted BETWEEN '"
				+ from + "' AND '" + to + "' AND currency = '" + currency1 + "' AND cashin.driverId=driver.driverId";
		try {
			...
			if (rows > 0) {
				...
				String query1 = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM cashin,driver WHERE cashin.datePosted BETWEEN '"
						+ from + "' AND '" + to + "' AND currency = '" + currency1
				...
				new DateRangePDF().generatePDF(table, download, from, to, Double.toString(Money.toMoney(total)),
						currency1);
