...
public class SingleDateSearch {

	ResultSet rs, rs1;
	Statement stm, stmt;
	double total = 0;

	public SingleDateSearch(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		super();
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
	...
	public void searchData(String datePosted, String currency1) {
		String query = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM cashin,driver WHERE cashin.datePosted = '"
				+ datePosted + "' AND currency='" + currency1 + "' AND cashin.driverId=driver.driverId ";
		try {
			...
			if (rows > 0) {
				...
				String query1 = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM cashin,driver WHERE cashin.datePosted = '"
						+ datePosted + "' AND currency='" + currency1 + "' AND cashin.driverId=driver.driverId ";
				...
				new SingleDatePDF().generatePDF(table, download, datePosted, Double.toString(Money.toMoney(total)),
						currency1);
