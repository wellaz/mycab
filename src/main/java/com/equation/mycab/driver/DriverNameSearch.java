...
public class DriverNameSearch {
	ResultSet rs, rs1;
	Statement stm, stmt;
	double total = 0;

	public DriverNameSearch(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		super();
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
	...
	public void searchData(String from, String to, String divername, String currency1) {
		String query = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM cashin,driver WHERE cashin.datePosted BETWEEN '"
				+ from + "' AND '" + to + "' AND firstName = '" + divername.split(" ")[0] + "' AND lastName = '"
				+ divername.split(" ")[1] + "' AND currency = '" + currency1 + "' AND cashin.driverId=driver.driverId ";
		try {
			...
			if (rows > 0) {
				...
				table.addContainerProperty("Driver Name", String.class, null);
				...
				String query1 = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM cashin,driver WHERE cashin.datePosted BETWEEN '"
						+ from + "' AND '" + to + "' AND firstName = '" + divername.split(" ")[0] + "' AND lastName = '"
						+ divername.split(" ")[1] + "' AND currency = '" + currency1
						+ "' AND cashin.driverId=driver.driverId ";
				...
				new DriverNamePDF().generatePDF(table, download, from, to, Double.toString(Money.toMoney(total)),
						divername, currency1);
				...
				new Notification(
						"<h1 style='color:white;'>We have searched all records and did not find any transaction for this period "
								+ from + " AND " + to
