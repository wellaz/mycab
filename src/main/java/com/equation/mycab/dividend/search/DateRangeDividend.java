...
public class DateRangeDividend {
	ResultSet rs, rs1;
	Statement stm, stmt;
	double total = 0;
	String name = null;

	public DateRangeDividend(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		super();
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
	...
	public void searchData(String from, String to) {
		String query = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM partner,dividend WHERE dividend.datePosted BETWEEN '"
				+ from + "' AND '" + to + "' ";
		try {
			...
			if (rows > 0) {
				...
				String query1 = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM partner,dividend WHERE dividend.datePosted BETWEEN '"
						+ from + "' AND '" + to + "' ";
				...
				new PartnerDateRangePDF().generatePDF(table, download, from, to, Double.toString(Money.toMoney(total)));
