...
public class PartnerNamePDF {

	public PartnerNamePDF() {
		super();
		return;
	}

	@SuppressWarnings({})
	public void generatePDF(Table table, Button download, String from, String to, String total, String partnername) {
		try {
			...
			document.add(new Paragraph("Period " + from + "And" + to + " Dividend Statement",
					FontFactory.getFont(FontFactory.TIMES_ROMAN, 9, Font.ITALIC, BaseColor.BLACK)));
			document.add(new Paragraph("Partner " + partnername,
					FontFactory.getFont(FontFactory.TIMES_ROMAN, 9, Font.ITALIC, BaseColor.BLACK)));
