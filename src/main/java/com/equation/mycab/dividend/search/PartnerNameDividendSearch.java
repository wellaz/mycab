...
public class PartnerNameDividendSearch {
	ResultSet rs, rs1;
	Statement stm, stmt;
	double total = 0;

	public PartnerNameDividendSearch(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		super();
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
	...
	public void searchData(String from, String to, String partnername) {
		String query = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM partner,dividend WHERE cashin.datePosted BETWEEN '"
				+ from + "' AND '" + to + "' AND firstName = '" + partnername.split(" ")[0] + "' AND lastName = '"
				+ partnername.split(" ")[1] + "'";
		try {
			...
			if (rows > 0) {
				...
				String query1 = "SELECT firstName,lastName,amount,currency,datePosted,timePosted,receiptId FROM partner,dividend WHERE cashin.datePosted BETWEEN '"
						+ from + "' AND '" + to + "' AND firstName = '" + partnername.split(" ")[0]
						+ "' AND lastName = '" + partnername.split(" ")[1] + "'";
				...
				new PartnerNamePDF().generatePDF(table, download, from, to, Double.toString(Money.toMoney(total)),
						partnername);
