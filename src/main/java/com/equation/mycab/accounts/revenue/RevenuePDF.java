...
public class RevenuePDF {
	...
	@SuppressWarnings({})
	public void generatePDF(Table table, Button download, String balance) {
		try {
			String filename = balance + "_statement" + ".pdf";
			...
			document.add(new Paragraph(" Revenue Acccount Statement",
					FontFactory.getFont(FontFactory.TIMES_ROMAN, 9, Font.ITALIC, BaseColor.BLACK)));
			document.add(new Paragraph("Dated: " + new DateUtility().getDate() + " at " + new DateUtility().getTime(),
					FontFactory.getFont(FontFactory.TIMES_ROMAN, 9, Font.ITALIC, BaseColor.BLACK)));
			document.add(new Paragraph("Current Balance $" + balance,
					FontFactory.getFont(FontFactory.TIMES_ROMAN, 9, Font.ITALIC, BaseColor.BLACK)));
